...
The autocorrelation of residuals is a way of examining if the residuals at time 1 are related to residuals at time 2. The the two are unrelated the rows are independent - and the regression assumption is true.
...
The textbook autcorrelation formula would give you the actual correlation between the two columns below which is .62. For technical R actually uses a slightly different calculation formula, above, which is conceptually similar, but produces a value of 0.56.
